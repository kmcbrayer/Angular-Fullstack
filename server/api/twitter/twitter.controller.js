...
exports.statuses = function(store){
  return function(req,res) {
    if (JSON.parse(store.getItem('twitter_cache')) === null) {
      log.info("did not hit 'twitter_cache' ");
      if(JSON.parse(store.getItem('twit_data')) !== null) {
        T.get(
          ...
          function(err, data, response) {
            if (!err) {
              ...
              for (var i=0;i<data.length;i++) {
                var tw = {};
                tw.type = 'twitter';
                tw.date = moment(data[i].created_at)._d;
